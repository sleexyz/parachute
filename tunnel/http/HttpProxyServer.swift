...
class HttpProxyServer {
        ...
        self.bootstrap = ServerBootstrap(group: group)
            ...
            .childChannelInitializer { channel in
                channel.pipeline.addHandler(ByteToMessageHandler(HTTPRequestDecoder(leftOverBytesStrategy: .forwardBytes)))
                    ...
                    .flatMap { channel.pipeline.addHandler(HttpHandler(logger: Logger(label: "com.strangeindustries.slowdown.HttpHandler"))) }
            }
    ...
    func start() {
        self.bootstrap.bind(to: try! SocketAddress(ipAddress: "127.0.0.1", port: 8080)).whenComplete { result in
            ...
            let logger = Logger(label: "com.strangeindustries.slowdown.HttpProxyServer")
            switch result {
