...
export function updateFdate(fdate: Date, points: number, now: Date = new Date()): Date {
...
export function pointsToFdate(points: number, now: Date = new Date()): Date {
    return new Date(now.getTime() +  (-1.0*Math.log(1 + points)/Math.log(lambda) * 1000));
}

export function fdateToPoints(fdate: Date, _now: Date = new Date()): number{
	return Math.pow(lambda,(_now.getTime() - fdate.getTime())/ 1000) - 1
}
