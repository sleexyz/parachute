...
func TestAppUpdateUsagePointsAboveThreshold(t *testing.T) {
	...
	c.SetSettings(&proxyservice.Settings{DefaultPreset: &proxyservice.Preset{BaseRxSpeedTarget: 1e6, UsageHealRate: 0.5, UsageMaxHP: 6}})
	now := time.Now()
	a := c.apps[0]
...
func TestAppUsagePointsRespondsToSettingsUpdate(t *testing.T) {
	...
	a.AddTxPoints(1.0, &now)
	c.sm.SetSettings(&proxyservice.Settings{DefaultPreset: &proxyservice.Preset{UsageHealRate: 1, UsageMaxHP: 3}})
	c.UpdateUsagePoints(time.Minute)
	a.ResetSampleState()
